...
public class P6 {
	...
	public static final int RESULT_CODE_GEN_ERROR = 4;
	...
	/**
	 * Used in main method to create a new instance.
	 * 
	...
	public int process() {
		...
		if (ErrMsg.getErr()) {
			return P6.RESULT_CODE_GEN_ERROR;
		}

		// Unparse for debugging purpose
		int debug = 0;
		if (debug == 1) {
			PrintWriter debugUnparseOutput;
			try {
				debugUnparseOutput = new PrintWriter("unparse.out");
				astRoot.unparse(debugUnparseOutput, 0);
				debugUnparseOutput.close();
			} catch (Exception e) {
				System.out.println("Unexpected exception in P6!");
			}
		}
